...
A Node.js + TypeScript backend for a kitchen renovation assistant in Linq iMessage, RCS, and SMS conversations. Contractors and homeowners can text in the same group; FORM asks useful questions, reads supported kitchen photos, maintains a project brief, and queues work for a person to review.

The dialogue follows contractor/homeowner triage → photos and discovery → brief confirmation → design revisions → proposal → procurement. The editable conversation prompt lives in [`prompts/designer.md`](prompts/designer.md).

## What works

- Signed Linq webhooks, sender-aware group conversations, and replies to the originating chat.
- OpenAI Responses with structured decisions and image/PDF input. Default model: `gpt-5-mini`.
...
- A local chat command that exercises the same backend without sending real texts.
- Render Blueprint, PostgreSQL, health checks, graceful shutdown, and deployment after GitHub checks pass.

This is the conversational foundation. Design, proposal, and procurement handoffs create database records for an operator. They do **not** generate 3D models, fetch live catalog prices, produce binding proposals, buy materials, notify an external team, or place calls. Operator replies can share reviewed design links and quotes. Those integrations can be added behind the handoff workflow.
...
Edit `.env`: set `OPENAI_API_KEY`, and replace `ADMIN_API_KEY` with a random token of at least 32 characters (`openssl rand -hex 32`). The example database URL matches Docker Compose. Leave `MESSAGING_MODE=sandbox` for local testing; no Linq credentials are needed.

```sh
npm run dev
```

Migrations run automatically on startup. In another terminal in this directory:

```sh
npm run chat
```

Try `/as contractor`, introduce yourself and the homeowner, then `/as homeowner` and describe the kitchen. `/new` starts another project; `/quit` exits. The chat uses your OpenAI key and incurs model usage, but never sends Linq messages. Point it at a deployed sandbox with `AGENT_URL=https://YOUR-SERVICE.onrender.com npm run chat` and that service's admin token in your local environment.
...
3. Confirm `/healthz` and `/readyz` return 200. Retrieve the generated admin token from Render's environment settings to use the chat command or admin API.
4. Enable Linq as described below. Keep the service at one instance initially; the database queue supports safe per-chat locking during rolling deployments.
...
All `/api/*` endpoints require `Authorization: Bearer <ADMIN_API_KEY>`. Treat this as an operator-only credential; don't embed it in a customer-facing app. Health checks are public; webhooks use Linq signature verification instead.
...
| POST | `/api/sandbox/messages` | Test a conversation with no real text delivery |
| GET | `/api/conversations` | Latest 100 conversations |
| GET | `/api/conversations/:id` | Brief, latest 40 messages, and latest 30 handoffs |
...
The request handler verifies the raw webhook signature and persists the inbound event before returning 200. A worker in the same process groups short message bursts, reads the latest history and saved brief, asks the model for a structured decision, saves it, sends the reply, then records completion.
...
The prompt is reread on each turn, so local prompt edits take effect immediately. Run `npm run chat` with `/new` to compare a fresh conversation. In production, commit and push to `main` to send changes through CI and Render.
