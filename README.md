...
In live chats, longer agent replies split into separate texts at sentence/paragraph boundaries, aiming for about 220 characters per text while keeping words, links, and emoji intact. Short replies stay together. Bubbles send consecutively as soon as the reply is ready, without artificial pauses; generated images accompany only the first text. Admin command menus and manually composed operator replies stay intact. The sandbox retains one logical reply and never sends live typing events.

Typing runs alongside actual model or image-generation work and refreshes during longer work, including after design-progress messages. It stops when the response is ready, on silence, cancellation, or errors; ready bubbles and delivery retries do not restart it. Typing and emoji reactions run independently of reply delivery, with short provider timeouts. [Linq supports visible typing in iMessage chats](https://docs.linqapp.com/channel/imessage/guides/chats/typing-indicators/); SMS/RCS do not display it, and a first reply after an idle period may not show an indicator.

By default, incoming message bursts settle for 500 ms (`REPLY_DEBOUNCE_MS`) and idle workers check for work every 250 ms (`WORKER_POLL_MS`). New input, pause, and reset are still checked between bubbles; shortening pickup delays does not change the model or visual reasoning quality.
