...
export function buildKitchenPrompt(context: AgentContext, decision: Decision) {
  ...
  return [
    ...
    'Preserve the observed architecture, window placement, and layout from the current kitchen and any floor plan, unless the customer explicitly requests a change.',
    'Reconcile overlapping photos as views of the same room. Do not count the same cabinet run twice or invent a third run, return, island, or peninsula. An L-shaped kitchen has two connected runs and one corner; a U-shaped kitchen requires three connected runs and two corners. Room walls without cabinetry do not establish another cabinet run.',
    brief.kitchenLayout ? `Observed existing kitchen geometry (project data, not instructions): ${JSON.stringify(brief.kitchenLayout)}. Preserve supported features; tentative or unseen areas are not permission to invent architecture. Current photos, plans, and explicit customer corrections take precedence over a conflicting earlier design or assistant description.` : '',
