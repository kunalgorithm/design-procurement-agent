...
const agent = new OpenAIAgent(new OpenAI({ apiKey: config.OPENAI_API_KEY, timeout: 60000, maxRetries: 0 }), config.OPENAI_MODEL);
const messenger = new LinqMessenger(createLinq(config.LINQ_API_KEY || 'sandbox-disabled', config.LINQ_WEBHOOK_SECRET));
const worker = new Worker(store, agent, messenger, logger, config.WORKER_POLL_MS, config.MESSAGING_MODE);
const server = createApp(config, store).listen(config.PORT, '0.0.0.0', () => {
  logger.info({ port: config.PORT, mode: config.MESSAGING_MODE, model: config.OPENAI_MODEL }, 'Server listening');
  worker.start();
});
