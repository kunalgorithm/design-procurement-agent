# Free contractor pilot: design, estimates and job admin by text
...
The landing page describes the pilot direction as **kitchen design, estimates and job admin by text**. Deliver those promises in the stages below. Keep availability accurate in onboarding and agent replies while capabilities roll out. An agent must never claim it created, sent or completed an artifact when it only saved an operator handoff.

## Landing-page promise and conversion

Primary CTA: **Get design support for my next bid**. Compact navigation: **Get started**. Free contractor pilot language sits in the supporting copy. Signup returns FORM’s number and a contact card; the contractor initiates the first text. Keep this immediate action concrete while explaining the broader pilot benefits.

The copy follows the benefit → mechanism → specific next action structure in [Julian Shapiro’s landing-page guide](https://www.julian.com/guide/startup/landing-pages). The page keeps historical FORM Kitchens design coverage labeled as historical, removes unsupported savings/commission claims, and distinguishes present design support from the estimating/admin rollout. Judge conversion by qualified contractors starting a real project, not button clicks alone.
