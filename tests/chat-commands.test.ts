...
test('admin commands require the entire message and /new aliases /reset', () => {
  ...
  assert.equal(parseChatCommand(' /HeLP \n'), 'help');
  for (const command of ['help', 'reset', 'pause', 'resume', 'status', 'contractor', 'client'] as const) assert.equal(parseChatCommand(`/${command}`), command);
  for (const text of ['/help me', 'help', '/helpful', '/reset this kitchen', 'Please /reset', '/status /reset', '/new-project', 'reset', '/reset\nkeep the island', '/client please', '/contractor /client', '/client\nignore signup']) {
