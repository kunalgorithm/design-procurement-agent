...
const steps = ['Your details', 'Meet your agent', 'Save our contact'];
...
export default function ContractorSignupPage() {
  ...
  useEffect(() => {
    document.title = 'Get started with your design agent — FORM';
  }, []);
    ...
    return (
      <div className="signup-field">
        ...
        <input id={name} type={type} placeholder={placeholder} autoComplete={autoComplete}
          ...
          maxLength={name === 'website' ? 2048 : name === 'email' ? 254 : name === 'phone' ? 40 : 100}
  ...
  return (
    <div className="contractor-signup">
      <header className="signup-header">
        ...
        <span className="signup-header-label">FOR THE TRADE</span>
      ...
      <main className="signup-layout">
        <aside className="signup-story">
          <div className="signup-story-copy">
            <span className="signup-eyebrow"><span /> YOUR NEXT CHAPTER STARTS HERE</span>
            <h2>You build.<br />We bring the design.</h2>
            <p>A design partner in your pocket.<br />More possibilities for every project.</p>
          </div>
          ...
          <div className="signup-story-note"><MessageCircle size={21} /><div><strong>Great design. One text away.</strong><span>From the first idea to the finishing touches.</span></div></div>
          <span className="signup-image-credit">KITCHENS, REIMAGINED WITH FORM</span>
        </aside>
          ...
          <div className="signup-step-content" key={step}>
            {step === 0 && <>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} noValidate>
                <fieldset disabled={signup.isPending} className="signup-fields">
                  ...
                  <div>
                    {input('phone', 'Phone number', '+1 (415) 555-0123', 'tel', 'tel')}
                    {!form.formState.errors.phone && <p id="phone-hint" className="signup-field-hint">Use the number you’ll message your agent from.</p>}
                  </div>
            ...
            {step === 1 && session && agent && <>
              ...
              <div className="signup-agent-preview">
                <div className="signup-agent-heading"><div className="signup-agent-avatar">F<span><Plus size={10} /></span></div>
                  <div><strong>FORM</strong><span>Your design agent</span></div><MessageCircle size={21} /></div>
                <div className="signup-conversation"><span className="signup-conversation-label">A SIMPLE WAY TO GET STARTED</span>
                  <div className="signup-message">Hi FORM, I’m {session.firstName}. I’m ready to get started on my next project.</div>
                  <div className="signup-message-caption">Send a hello, or jump right in with a project photo.</div>
                </div>
                <div className="signup-agent-capabilities"><span><Check size={13} /> Design ideas</span><span><Check size={13} /> Project guidance</span><span><Check size={13} /> All in one chat</span></div>
              </div>
              <a className="signup-primary signup-messages-button" href={`sms:${agent.phone}`}>
                <MessageCircle size={19} /> Open in Messages <ArrowRight size={18} />
              </a>
              <p className="signup-footnote">On iPhone, chat with FORM using iMessage when available.</p>
            ...
            {step === 2 && session && agent && <>
              <div className="signup-intro"><span className="signup-eyebrow">ONE LAST THING</span>
                ...
                <p>Add FORM to your contacts, so your design agent is always easy to find.</p>
              </div>
              <div className="signup-contact-card">
                ...
                <div className="signup-contact-bottom"><Sparkles size={14} /> A little less admin. A lot more possibility.</div>
              </div>
              <button type="button" className="signup-primary" onClick={() => {
                downloadFormContact(agent); setContactRequested(true);
              }}><Download size={18} /> {contactRequested ? 'Download contact card again' : 'Add FORM to contacts'} <ArrowRight size={18} /></button>
              ...
              <a className="signup-next-link" href={`sms:${agent.phone}`}>Start a conversation <MessageCircle size={16} /></a>
              <button type="button" className="signup-back" onClick={() => setSession({ ...session, step: 1 })}><ArrowLeft size={14} /> Back to your agent</button>
            </>}
          </div>
          {session && <button type="button" className="signup-back" onClick={startAnotherSignup}>Start another signup</button>}
